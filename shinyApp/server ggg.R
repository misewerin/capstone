...
shinyServer(function(input, output) {
    ...
    getNGramPrediction <- function(inputSplit, wordNumber, gramsTable, threshold) {
        ...
        # calculate probability of next words
        gramsTableFiltered$prob <- (gramsTableFiltered$freq/sum(gramsTableFiltered$freq)) * (wordNumber + 1)
        # if no results left, return empty list
        if(nrow(gramsTableFiltered) == 0) {
            return(data.table(words = character(0), freq = integer(0), prob = double(0)))
        }
        # return numberReturnWords words ordered by descending probability
        gramsTableFiltered <- gramsTableFiltered[order(-gramsTableFiltered$prob),][1:min(nrow(gramsTableFiltered), numberReturnWords),]
    ...
    getNextWords <- function(inputString) {
        ...
        #prepare predictions
        predictions <- data.table(words = character(0), freq = integer(0), prob = double(0))
        ...
        # aggregate by word (ngram) and return the 3 with the highest probability
        predictionsAgg <- aggregate(prob ~ words, predictions, sum)
        predictionsAgg <- predictionsAgg[order(-predictionsAgg$prob),]
        return(setDT(predictionsAgg[1:min(length(predictionsAgg[,1]),3),])$words)
    }
